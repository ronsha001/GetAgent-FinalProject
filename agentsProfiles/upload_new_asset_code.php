<?php session_start();
    ...
    $file = array($_FILES['file1'], $_FILES['file2'], $_FILES['file3'], $_FILES['file4'], $_FILES['file4'], $_FILES['file5'], $_FILES['file6'], $_FILES['file7'], $_FILES['file8']);
    $file1 = $_FILES['file1'];
    $file2 = $_FILES['file2'];
    $file3 = $_FILES['file3'];
    $file4 = $_FILES['file4'];
    ...
    for($i = 1, $j = 0; $i < 9; $i++, $j++){
        if(!empty($file[$j]['name'])){
            $fileName = $file[$j]['name'];
            $fileTmpName = $file[$j]['tmp_name'];
            $fileSize = $file[$j]['size'];
            $fileError = $file[$j]['error'];
            $fileType = $file[$j]['type'];
    ...
    try{
        ...
        '$sale_or_rent', '$asset_type', '$asset_condition', '$city', '$house_number', '$street',
        '$floor', '$max_floor', '$num_of_rooms', '$balcony', '$size_in_sm', '$parking_station',
        '$entrance_date', '$check_boxes', '$price', '$tax', '$currency', '$asset_description', '$fileDestination[1]', '$fileDestination[2]',
        '$fileDestination[3]', '$fileDestination[4]', '$fileDestination[5]', '$fileDestination[6]', '$fileDestination[7]', '$fileDestination[8]')";
